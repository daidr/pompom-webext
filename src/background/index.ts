    ...
    if (Number.isInteger(data)) {
      ...
      if (!hasUpdatedBadge) {
        ...
        if (_selectedUid && _selectedUid === _copyItem.uid)
          shouldUpdateBadge = true // 更新 _selectedUid 当前的 stamina 数据到 badge
...
onMessage<{ uid: string }, 'request_captcha_bg'>(
  ...
  async ({ data: { uid } }) => {
    ...
    const curtab = await tabs.create({
      url: 'https://webstatic.mihoyo.com/app/community-game-records/rpg/?game_id=6&ref=pompom&mhy_presentation_style=fullscreen#/',
    })
