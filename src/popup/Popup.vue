...
const userDataList = ref<IUserDataItem[]>([])
...
const userData = computed<IUserDataItem>(() => {
...
<template>
  <div v-if="showCaptchaContainer" id="captcha-container" class="captcha-container" />
    ...
    <div v-if="Object.keys(userData).length > 0" class="main-wrapper">
        ...
        <div class="left">
          <p>
            ...
            <select @change="onSelectUidChange">
              <option v-for="item in userDataList" :key="item.uid" :value="item.uid" :selected="item.uid === selectedUid">
      ...
      <template v-if="!userData.isError">
        <div class="stamina-stats">
          <h2>
            <span class="stamina-title">
              <img src="/assets/starrail/power.png">
              {{ i18n.getMessage("popup_StaminaTitle") }}</span>
            <span class="update-time">{{ i18n.getMessage("popup_UpdateTimeTitle")
            }}{{ new Date(userData.updateTimestamp).toLocaleString() }}</span>
          </h2>
          <p class="stamina-num">
            {{ userData.data.current_stamina }}/{{ userData.data.max_stamina }}
          </p>
          <template v-if="userData.data.current_stamina < userData.data.max_stamina">
            <p class="sub-stat-item">
              ...
              <span class="right">
                <TimeComponent :time="getTime(Number(userData.data.stamina_recover_time))" />
              </span>
            </p>
            <p class="sub-stat-item">
              <span class="left">
              ...
              <span class="right">
                <DayComponent :time="getClock(Number(userData.data.stamina_recover_time))" />
              </span>
            </p>
          </template>
        </div>
        ...
        <div class="expeditions-stats">
          <h2 :class="{ 'has-result': userData.data.expeditions.length > 0 }">
            ...
            {{ userData.data.accepted_epedition_num }}/{{
              userData.data.total_expedition_num
            }}
          </h2>
          <template v-if="userData.data.expeditions.length > 0">
            <div v-for="(expedition, index) of userData.data.expeditions" :key="index" class="expedition-item">
              <div class="left flex-grow mt-4px">
                <div class="flex gap-1 justify-between items-center w-[95%] overflow-hidden">
                  <span class="text-13px overflow-hidden text-ellipsis whitespace-nowrap">{{ expedition.name }}</span>
                  <div class="flex flex-shrink-0">
                    <img v-for="avatar of expedition.avatars" :key="avatar" class="expedition-avatar" :src="avatar">
                  </div>
                </div>
              </div>
              <div class="flex flex-col gap-1 items-end">
                ...
                <div v-if="expedition.remaining_time !== '0'" class="right expedition-right">
                  <TimeComponent :time="getTime(Number(expedition.remaining_time))" />
                </div>
              </div>
            </div>
          </template>
        ...
        <div class="more-stats">
          <div class="stat-item">
            <span class="left">
              <img src="/assets/starrail/reserved_power.png">
              {{ i18n.getMessage("popup_ReservedStaminaTitle") }}
            </span>
            <span class="right">
              {{ userData.data.current_reserve_stamina }}
              <span v-if="userData.data.is_reserve_stamina_full">
                {{ i18n.getMessage("popup_ReservedStaminaFull") }}
              </span>
            </span>
          </div>
          <div class="stat-item">
            <span class="left">
              <img src="/assets/starrail/daily_training.png">
              {{ i18n.getMessage("popup_DailyTrainingTitle") }}
            </span>
            <span class="right">{{ userData.data.current_train_score }}/{{
              userData.data.max_train_score
            }}
            </span>
          </div>
          <div class="stat-item">
            <span class="left">
              <img src="/assets/starrail/universe_points.png">
              {{ i18n.getMessage("popup_UniversePoints") }}
            </span>
            <span class="right">{{ userData.data.current_rogue_score }}/{{
              userData.data.max_rogue_score
            }}
            </span>
          </div>
          <div class="stat-item">
            <span class="left">
              <img src="/assets/starrail/weekly_boss.png">
              {{ i18n.getMessage("popup_WeeklyBossesTitle") }}
            </span>
            <span class="right">{{
              userData.data.weekly_cocoon_cnt
            }}/{{ userData.data.weekly_cocoon_limit }}</span>
          </div>
        </div>
      </template>
    </div>
...
.main-wrapper {
  ...
  .stamina-stats {
    ...
    .stamina-num {
      ...
      font-family: number-mono !important;
    }
  ...
  .expeditions-stats {
    ...
    .expedition-item {
      ...
      img {
        @apply h-5 w-5;
        @apply rounded-full overflow-hidden border border-1 border-[#e6decccc];
      }
